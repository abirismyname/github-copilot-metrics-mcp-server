...
export function validateOrganizationName(org: string): void {
  if (!org || typeof org !== 'string') {
    throw new ValidationError('Organization name is required and must be a string', 'org');
  }
  
  if (org.length < 1 || org.length > 39) {
    throw new ValidationError('Organization name must be between 1 and 39 characters', 'org');
  }
  
  if (!/^[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?$/.test(org)) {
    throw new ValidationError('Organization name contains invalid characters', 'org');
  }
}
...
export function validateUsername(username: string): void {
  ...
  if (username.length < 1 || username.length > 39) {
    throw new ValidationError('Username must be between 1 and 39 characters', 'username');
  }
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (error instanceof ValidationError || 
          (error instanceof GitHubAPIError && [401, 404].includes(error.statusCode || 0))) {
        logger.debug(`Not retrying ${operationName} due to error type`, { error: error.message });
        throw error;
      }
